...
def window_partition(x, window_size):
    ...
    if H < window_size:
        window_size /= 2
        window_size = torch.tensor(window_size, dtype=torch.int64)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask = None):
        ...
        if mask is not None:
            nW = mask.shape[0]
            attn = attn.view(B_ // nW, nW, self.num_heads, N, N) + mask.unsqueeze(1).unsqueeze(0).cuda()
...
class SwinTransformerBlock(nn.Module):
    ...
    def generate_mask(self,h=0,w=0):
    ...
    def forward(self, x):
        ...
        # W-MSA/SW-MSA
        attn_mask = self.generate_mask(H,W)
...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        layer_x = format_reverse(layer_x)
        # for i in layer_x:
        #     print(i.shape)
        return layer_x
...
def format_reverse(x):
    if isinstance(x, (list, tuple)):
       ...
       if conv_in:
           ...
           res = [rearrange(x0, 'b (h w) c -> b c h w', h=int(math.sqrt(x0.shape[-2]))) for x0 in x]
    else:
       conv_in = False if len(x.shape) == 3 else True
       if conv_in:
           res = rearrange(x, 'b c h w -> b (h w) c')
       else:
           res = rearrange(x, 'b (h w) c -> b c h w', h=int(math.sqrt(x.shape[-2])))
    return res
...
if __name__ == '__main__':
    ...
    model = swin_backbone(window_size=12, img_size=384, num_classes=200).cuda()
    # model = swin_backbone_tiny(window_size=12, img_size=384, num_classes=200)
    # print(model)
    y = model(x2)
    for i in y:
        print(i.shape)
